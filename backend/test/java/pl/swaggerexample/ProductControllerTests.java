...
public class ProductControllerTests {
    ...
    public void init() throws Exception {
        List<Product> products = Arrays.asList(
            ...
            new Product("Butter", "500g of butter.", Collections.singleton("/butterimageurl"), BigDecimal.valueOf(7.29D),
                        8));
    ...
    public void getProductByInvalidIdReturnNotFound() throws Exception {
        mockMvc.perform(get("/api/products/{id}", 999L).with(user(USER.build()))).andDo(print())
    ...
    public void getProductsFilteredByCustomPredicatesReturnOk(SimpleImmutableEntry<String, String> param) throws Exception {
        mockMvc.perform(get("/api/products").queryParam(param.getKey(), param.getValue()).with(user(USER.build())))
               .andDo(print())
               .andExpect(status().isOk())
    ...
    public void deleteProductWithAuthorizationReturnNoContent() throws Exception {
        mockMvc.perform(delete("/api/products/{id}", 1L).with(user(MANAGER.build()))).andDo(print())
               .andExpect(status().isNoContent());
    }

    @Test
    public void deleteProductWithoutAuthorizationReturnForbidden() throws Exception {
        mockMvc.perform(delete("/api/products/{id}", 1L).with(user(USER.build()))).andDo(print())
               .andExpect(status().isForbidden());
    }

    @Test
    public void deleteProductWithInvalidIdReturnNotFound() throws Exception {
        mockMvc.perform(delete("/api/products/{id}", 999L).with(user(MANAGER.build()))).andDo(print())
               .andExpect(status().isNotFound());
    }
}