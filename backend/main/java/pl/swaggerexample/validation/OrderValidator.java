...
public class OrderValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        Set<Long> orderedProductsIds = order.getItems().stream()
                                            .filter(item -> item.getProduct().getId() != null)
                                            .map(item -> item.getProduct().getId()).collect(Collectors.toSet());

        Map<Long, Integer> productQuantitiesFetchedFromDb =
            StreamSupport.stream(productService.getProductsByIds(orderedProductsIds).spliterator(), false)
        ...
        for (OrderItem orderItem : order.getItems()) {
            ...
            if (productQuantityFromDb == null) {
                errors.rejectValue("items[" + itemIndex + "]", "",
                                   String.format("Product with ID=%s doesn't exist.", orderItem.getProduct().getId()));
