...
public class OrderService implements EntityService<Order>
{
	...
	public List<Order> getOrdersByBuyerId(Long buyerId)
	{
		...
		{
			if (!currentUser.getDetails().equals(buyerId)) throw new AccessDeniedException("Invalid ID.");
		}
		
		if (!userDao.existsById(buyerId)) throw new NotFoundException("There is no user with id: " + buyerId);
